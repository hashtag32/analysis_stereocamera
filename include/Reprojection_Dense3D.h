#pragma once
class Reprojection_Dense3D
{
public:
	Reprojection_Dense3D();
	~Reprojection_Dense3D();
};
...
namespace cl
{
	...
	private:
		void init()
		{
			...
			glutDisplayFunc([](){ _this->draw(); });
			glutMouseFunc([](int button, int state, int x, int y){ _this->mouse(button, state, x, y); });
			glutKeyboardFunc([](unsigned char c, int x, int y){ _this->key(c, x, y); });
			glutMotionFunc([](int x, int y){ _this->motion(x, y); });
			glutReshapeFunc([](int width, int height){ _this->reshape(width, height); });
		...
		void reshape(int width, int height)
		{
			...
			glMatrixMode(GL_PROJECTION);
			glLoadIdentity();
			gluPerspective(75.0, windowWidth / windowHeight, 0.001, 20000.0);
			glMatrixMode(GL_MODELVIEW);
		}
		void drawPointCloud()
		{
